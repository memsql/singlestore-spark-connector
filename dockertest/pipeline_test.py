...
def test_kafka_checkpointing_multiple_partitions(local_context):
    ...
    assert checkpoint_data['broker'] == "%s:9092" % ctx.external_ip
    ...
    producer.produce(['{"num": %d}' % i for i in range(50, 100)])
    ...
    rows = map(lambda x: int(x.num), conn.query("SELECT data::$num AS num FROM db.t"))
    assert sum(rows) > sum(range(50)), "incorrect row data: %d but expected more than %d" % (sum(rows), sum(range(50)))
