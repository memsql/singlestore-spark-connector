...
case class BitAggregateExpressionExtractor(expressionExtractor: ExpressionExtractor,
  ...
  def unapply(f: AggregateFunction): Option[Statement] = {
    f match {
      // BitAnd.scala
      case BitAndAgg(expressionExtractor(child)) if context.singlestoreVersionAtLeast("7.0.1") =>
      ...
      // BitXor.scala
      case BitXorAgg(expressionExtractor(child)) if context.singlestoreVersionAtLeast("7.0.1") =>
