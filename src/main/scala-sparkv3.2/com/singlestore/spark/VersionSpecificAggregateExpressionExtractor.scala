...
case class VersionSpecificAggregateExpressionExtractor(expressionExtractor: ExpressionExtractor,
  ...
  def unapply(aggFunc: AggregateFunction): Option[Statement] = {
    aggFunc match {
      ...
      case Kurtosis(expressionExtractor(child), nullOnDivideByZero) if nullOnDivideByZero =>
      ...
      case Skewness(expressionExtractor(child), nullOnDivideByZero) if nullOnDivideByZero =>
