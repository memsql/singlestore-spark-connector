...
class BatchInsertTest extends IntegrationSuiteBase with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  it("insert into a new table") {
    ...
    val actualDF =
      spark.read.format(DefaultSource.MEMSQL_SOURCE_NAME_SHORT).load("testdb.batchinsertnew")
  ...
  def insertAndCheckContent(batchSize: Int,
    ...
    val actualDF =
      spark.read.format(DefaultSource.MEMSQL_SOURCE_NAME_SHORT).load("testdb.batchinsert")
