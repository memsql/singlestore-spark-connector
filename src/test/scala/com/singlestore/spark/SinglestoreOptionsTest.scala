...
class SinglestoreOptionsTest extends IntegrationSuiteBase {
  val requiredOptions = Map("clusterEndpoints" -> "h:3306")

  describe("equality") {
    it("should sort clusterEndpoints") {
      assert(
        SinglestoreOptions(
          CaseInsensitiveMap(Map("clusterEndpoints" -> "host1:3302,host2:3302,host1:3342"))) ==
          SinglestoreOptions(
            CaseInsensitiveMap(Map("clusterEndpoints" -> "host1:3302,host2:3302,host1:3342"))),
        "Should sort clusterEndpoints"
      )
    }
  }
