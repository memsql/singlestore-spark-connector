...
## Version: 5.0.0 [![License](http://img.shields.io/:license-Apache%202-brightgreen.svg)](http://www.apache.org/licenses/LICENSE-2.0.txt)
...
```
$SPARK_HOME/bin/spark-shell --packages com.singlestore:singlestore-spark-connector_2.12:5.0.0-spark-4.0.0
```
...
For example, in connector `5.0.0-spark-4.0.0`, 4.1.11 is the version of the connector, 
...
| `customSchema`                                    | -                                                     | Custom schema to use when reading data. For example: `id DECIMAL(38,0), name STRING`. When set, the connector uses this schema instead of querying the database for metadata, which can improve performance when schema inference from the database is expensive or unnecessary. This option is incompatible with SQL pushdown; `disablePushdown` must be set to true.
